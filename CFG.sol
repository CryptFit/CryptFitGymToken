...
contract CFG is Context, IERC20, Ownable {
    ...
    string private _name = "Crypt Fit Gym Token";
    ...
    address payable public marketingWallet = payable(0xa6481B0334d7F50ad39bd1C7d8475870dD22Bd57);
    address payable  public rewardWallet = payable(0x3b0CA8E2AAebaEb6F081522a7A8ceD45EaE2b15F);
    ...
    uint256 public numTokensSellToAddToLiquidity = 300000 * 10**6;
    ...
    constructor() {
        ...
        _isExcludedFromMaxTx[rewardWallet] = true;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _burnFee==0 && _rewardFee==0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_marketingFee).add(_rewardFee); // marketing + reward + liquidity fees
        
        
        
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees); 
        uint256 tokenreward = contractTokenBalance.mul(_rewardFee).div(combineFees);
        
        uint256 tokensForMarketing = contractTokenBalance.sub(tokensForLiquidity).sub(tokenreward);
        uint256 tokensForreward = contractTokenBalance.sub(tokensForLiquidity).sub(tokensForMarketing);
        ...
        swapTokensForEth(tokensForreward);
        ...
        transferToAddressETH(rewardWallet, transferredBBalance);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private 
    {
        ...
        (tTransferAmount, rTransferAmount) = takereward(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
    ...
    function takereward(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
        if(_rewardFee==0) {  return(tTransferAmount, rTransferAmount); }
        ...
        emit Transfer(sender, address(this), treward);
        return(tTransferAmount, rTransferAmount); 
    }
    ...
    function setTaxFeePercent(uint256 taxFee,uint256 liquidityFee,uint256 MarketingFee,uint256 burnFee, uint rewardFee) external onlyOwner() {
        _taxFee = taxFee;
        _pretaxfee = _taxFee;
        ...
        _prerewardfee = rewardFee;
        _burnFee = burnFee;
        _preburnfee = _burnFee;
    }
    
    function setsellTaxFeePercent(uint256 taxsellFee,uint256 liquiditysellFee,uint256 MarketingsellFee,uint256 burnsellFee, uint256 rewardsellFee) external onlyOwner() {
